...
(function (global, factory) {
  ...
  var TagCloud = /*#__PURE__*/function () {
    ...
    function TagCloud() {
      ...
      // choose radius by window size
      self.radius_coe = 3;
      self.radius = (window.innerWidth + window.innerHeight) / self.radius_coe; // rolling radius

      self.img_size_coe = 6;
      self.img_size = self.radius / self.img_size_coe; // img size

      self.depth_coe = 1.0; // depth coefficient
      self.depth = self.depth_coe * self.radius; // rolling depth
    ...
    _createClass(TagCloud, [{
      ...
      value: function _createItemElement(html) {
        ...
        if (self.config.useItemInlineStyles) {
          ...
          itemEl.style.top = '49%';
          itemEl.style.left = '47.5%';
          itemEl.style.zIndex = -2;
          ...
          if (img_el) {
            ...
            img_el.style.width = self.img_size + 'px';
            img_el.style.height = self.img_size + 'px';
          }

          var text_el = itemEl.querySelector('center');
          if (text_el) {
            // set background color of text
            text_el.style.backgroundColor = 'rgb(35, 37, 47)';
          }
      ...
      value: function _init() {
        ...
        if ("ontouchstart" in document.documentElement) {
          ...
          TagCloud._on(window, 'resize', function () {
            console.log('resize');
            var last_radius = self.radius;
            self.radius = (window.innerWidth + window.innerHeight) / self.radius_coe;
            self.img_size = self.radius / self.img_size_coe;
            self.depth = self.depth_coe * self.radius;
            self.size = 1.5 * self.radius;

            // scale item x, y, z based on new radius
            self.items.forEach(function (item) {
              var per = self.radius / last_radius;
              item.x *= per;
              item.y *= per;
              item.z *= per;
            });

            // update items' img size
            self.items.forEach(function (item) {
              var img_el = item.el.querySelector('img');
              if (img_el) {
                img_el.style.width = self.img_size + 'px';
                img_el.style.height = self.img_size + 'px';
              }
      ...
      value: function _next() {
        ...
        // The larger the item.z, the farther away it is from the screen

        // sort items based on item.z
        self.items.sort(function (a, b) {
          return b.z - a.z;
        });
        // update items' z-index based on item.z
        self.items.forEach(function (item, index) {
          item.el.style.zIndex = index - self.items.length - 1;
        });

        // ban items' click event when the item is far away from the screen
        self.items.forEach(function (item) {
          if (item.z > 0) {
